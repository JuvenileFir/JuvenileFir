...
struct root
	{
    persistent_ptr<base_t[]> pda;
    persistent_ptr<ske_t[]> pcs;
	};
  
  persistent_ptr<base_t[]> pptr;
  persistent_ptr<ske_t[]> pcsptr;
  pool<root> pop;
...
ske_t map_it(base_t num) {
  ...
  while (high < SKE_N) {
    base_t tempdata = map_array[high][0];
    int option = map_array[high][1];
    bool break_loop = false;

    switch (option) {
      case NON_UNIQUE_CODE:
        break_loop = (num <= tempdata);
        break;
      case UNIQUE_CODE:
        break_loop = (num == tempdata);
        break;
...
void le_scan_work(int t_id, int bitmap_res_tmp[][2]) {
  ...
  while (cur < DATA_N){
    ...
    for (int j = 0; j < STRIDE; j++) {
      ...
      //对每个代码执行谓词求值所需的逻辑比较
      //从i处开始，从数组连续载入32个值
      __m256i code = _mm256_loadu_si256((__m256i*)&pcsptr[i]);// ((__m256i*)&ske_col[i]);
...
void gt_scan_work(int t_id, int bitmap_res_tmp[][2]) {
  ...
  while (cur < DATA_N){
    ...
    for (int j = 0; j < STRIDE; j++) {
      int i = cur + j * 32;      
      __m256i code = _mm256_loadu_si256((__m256i*)&pcsptr[i]);
    ...
    if (!is_unique_1 || !is_unique_2) {
      for (int i = 0; i < bitmap_res_tmp_idx; i++) {
        ...
        while (bitmap) {
          ...
          if (data_pos < DATA_N && (mask & bitmap_res_tmp[i][1]) &&
          pptr[data_pos] > TARGET_NUMBER &&
          pptr[data_pos] < TARGET_NUMBER_2)//后续添加，源程序似乎遗漏此条件
            bitmap_res[bitmap_pos] |= mask;
...
void init(int argc, char* argv[]) {  
  char optstr[] = "n:f:l:r:i:p:T:w:s:o:d";
  extern char* optarg;
  char opt;
  ...
  while ((opt = getopt(argc, argv, optstr)) != -1) {
    switch (opt) {
      ...
      case 'r':
        ...
        break;
      case 'i':
        strcpy(APPEND_PATH, optarg);
        break;
      case 'p':
        percentage = strtouint32(optarg);
        break;
      case 'T':
...
//确保右端点数组为空，或左右端点数组大小相等
  ...
  BITMAP_N = ROUNDUP_DIVIDE(MALLOC_DATA_N, 32);  // 结果为data数/stride，向上取整；int bitmap_res[]
    ...
    //初始化pool及root，返回pool<root>变量pop
    if (access(pmempath, F_OK) == 0){
			pop = pool<root>::open(pmempath, LAYOUT);
      printf("已有\n");}
	else{
...
//下述结构参数但凡有一个未设置（即=0），则打印失败信息，并退出程序
  if (!samples || !res || !bitmap_res) {
  ...
  while (1) {
    ...
    if (tmp < ratio) {
      ...
      if (gened_samples_n < Samples_n)
        samples[gened_samples_n++] = pptr[cur];    //samples[gened_samples_n++] = data[cur];
      else
        break_loop = true;
...
void init_data_from_file() {
  ...
  if (!(fp = fopen(DATA_PATH, "rb"))) {
    printf("init_data_from_file: 文件%s 打开失败\n", DATA_PATH);
    exit(-1);
  }  
  ...
  printf("开始从文件：%s 装载数据 \n", DATA_PATH);
   gettimeofday(&t1, NULL);
    ...
    if(!(fread(file_data, sizeof(uint8_t), DATA_N, fp))) exit(-1);    
    transaction::run(pop, [&] {
    ...
    if(!(fread(file_data, sizeof(uint16_t), DATA_N, fp))) exit(-1); 
    transaction::run(pop, [&] {
          for (int i = 0; i < size; i++) pptr[i] = file_data[i];
		});
    free(file_data);
    ...
    printf("装载数据完成，用时： %f ms\n",  elapsed);//显示程序运行时间    
  JOBS_PER_THREAD = ROUNDUP(ROUNDUP_DIVIDE(size, THREAD_N), 32);
  // ReservoirSample(data, DATA_N, samples, SAMPLES_N);
  printf("data数据量：%d   每线程任务数：%d\n\n开始采样\n",size,JOBS_PER_THREAD);
...
void* create_ske_col_work(void* argument) {
  ...
  for (int i = start; i < start + jobs && i < DATA_N; i++)
    pcsptr[i] = map_it(pptr[i]); //ske_col[i] = map_it(pptr[i]);
...
void check_worker(base_t* codes, int n, int* bitmap, base_t target1, base_t target2, int t_id) {
  ...
  for (int i = start; i < end; i++) {
    base_t tempdata = codes[i];
    int truth;
    switch (OPERATOR_TYPE) {
      case OPERATOR_EQ:
        truth = tempdata == target1;
        break;
      case OPERATOR_LE:
        truth = tempdata < target1;
        break;
      case OPERATOR_GT:
        ...
        break;
      case OPERATOR_BET:
        truth = tempdata > target1 && tempdata < target2;
        break;
      default:
        assert(0);
    }
...
int restore_and_check(base_t target, bool in_res) {
  ...
  // check res[]
  for (int i = 0; i < DATA_N; i++)
    if (((OPERATOR_TYPE == OPERATOR_LE) &&
         ((res[i] && pptr[i] >= target) || (!res[i] && pptr[i] < target))) ||
        ((OPERATOR_TYPE == OPERATOR_EQ) &&
         ((res[i] && pptr[i] != target) || (!res[i] && pptr[i] == target))) ||
        ((OPERATOR_TYPE == OPERATOR_GT) &&
         ((res[i] && pptr[i] <= target) || (!res[i] && pptr[i] > target)))) {
      printf("data[%d]: %u failed. res: %d, ground truth: %d\n", 
              ...
              pptr[i] < target);
      printf("map_it(target): %d, map_it(failed): %d\n",
             SKE_CODE_2_POS(map_it(target)),
             SKE_CODE_2_POS(map_it(pptr[i])));
      return -1;
    }
...
void* naive_scan_work(void* argument) {//实际工作
  ...
  for (int i = start; i < start + jobs && i < DATA_N; i++) {
    ske_t candidiate = pcsptr[i];    //ske_t candidiate = ske_col[i];
    if (candidiate < ske_code)
      res[i] = 1;
    else if (candidiate == ske_code)//编码相等
      res[i] = pptr[i] < target;//则再于上层比较大小   res[i] = data[i] < target;
  }
...
/*void simd_scan(base_t target) {
    ...
    for (int j = 0; j < STRIDE; j++) {
      int i = cur + j * 32;//每次处理一个数据块的代码
      //对每个代码执行谓词求值所需的逻辑比较
      __m256i code = _mm256_loadu_si256((__m256i*)&pcsptr[i]);//从i处开始，从数组连续载入32个值   //__m256i code = _mm256_load_si256((__m256i*)&ske_col[i]);
      __m256i definite = _mm256_cmpgt_epi8(repeat_sx_256_1, code);
      int bitvector_def = _mm256_movemask_epi8(definite);
      //将32个字节的最高有效位（1/0）提取出来组合为一个32位int数，即变成位向量
      bitmap_res[i / 32] = bitvector_def;//前面数组中的下标实际为1、2、3……
    ...
    if (!is_unique_1) {
      for (int i = 0; i < bitmap_res_tmp_idx; i++) {
        ...
        while (bitmap) {
          ...
          int data_pos = (bitmap_pos << 5) + j;//换算出第一个相等的位向量编码位置
          if (data_pos < DATA_N && (mask & bitmap_res_tmp[i][1]) &&              
            pptr[data_pos] < TARGET_NUMBER)   //data[data_pos] < TARGET_NUMBER)
            bitmap_res[bitmap_pos] |= mask;
            //↑注意此处相|的对象是bitmap_res数组，正常情况下其相等位置应全为0。故此操作为逐一单点置位
          bitmap = bitmap & (bitmap - 1);//与减了1的自己相与，效果为去掉一个最低位1
        }
...
void thread_simd_scan(base_t target) {
  ...
  for (int i = 0; i < THREAD_N; i++) {
    t_id[i] = i;
    if (pthread_create(&t[i], NULL, simd_scan_work, &t_id[i]) < 0) {
...
void test_scan(base_t target) {
  struct timeval t1, t2;
  double elapsed;
  // FILE* fp = fopen("./tpc_search2/search.xls", "a");
...
void append(){
  ...
  stat(DATA_PATH,&st);
  size=ROUNDUP_DIVIDE(st.st_size,4);
  if (size == DATA_N) {
      printf("append failed:原数据已满.\n");
      exit(-1);
    }

  stat(APPEND_PATH,&st);
  int size_i=ROUNDUP_DIVIDE(st.st_size,4) * percentage * 0.01;
  if (size_i == 0) {
      printf("append failed：插入数据为空.\n");
      exit(-1);
    }
  if (strlen(DATA_PATH)){
    FILE*fp;
    if (!(fp = fopen(APPEND_PATH, "rb"))) {
    printf("插入数据的文件：(%s) 打开失败\n", APPEND_PATH);
    exit(-1);
  }  
    uint32_t* file_data = (uint32_t*)malloc(size_i * sizeof(uint32_t));
    if (fread(file_data, sizeof(uint32_t), size_i, fp) == 0) {
    ...
    transaction::run(pop, [&]{
    for (int pi = 0; pi < size_i; pi++) pptr[size+pi] = file_data[pi];
    });
    ...
    printf("新data数据量:%d\t每线程任务数:%d\n",size,JOBS_PER_THREAD);
    
    MakeSamples();
    create_map();
    create_ske_col();//使用函数而非for循环，可以多线程实现
    }
    else{
    transaction::run(pop, [&]{
    for (int pi = 0; pi < size_i; pi++) pcsptr[size+pi]=map_it(file_data[pi]);          
    });
    size+=size_i;          
    }

    gettimeofday(&t2, NULL);
    fp = fopen("append.xls", "a");
    elapsed = (t2.tv_sec - t1.tv_sec) * 1000.0;//秒
    elapsed += (t2.tv_usec - t1.tv_usec) / 1000.0;//微秒
    fprintf(fp,"%.3f\n",elapsed);
    printf("append完成，用时： %f ms\n\n",  elapsed);//显示程序运行时间
    fclose(fp);
    free(file_data);    
    
    std::cout<<"插入数据量："<<size_i<<"  "<<"插入后总数据量："<<size<<std::endl;
  }
  else{
