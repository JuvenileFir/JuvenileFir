...
std::vector<base_t> target_numbers_l;  // left target numbers左端点
std::vector<base_t> target_numbers_r;  // right target numbers右端点
char OP[5];
...
typedef struct {
  simd_scan_args_t* inner;
  int t_id;
} create_ske_col_real_args_t;
...
//确保右端点数组为空，或左右端点数组大小相等
  ...
  JOBS_PER_THREAD = ROUNDUP(ROUNDUP_DIVIDE(DATA_N, THREAD_N),
                            32);  // 数据总数除以线程数，再以为单位32向上取整，即得每线程任务数
...
void simd_scan(base_t target) {
  ske_t ske_code = map_it(target);//设置目标数的S映射
  bool is_unique = map_array[SKE_CODE_2_POS(ske_code)][1] == UNIQUE_CODE;
  repeat_sx = _mm_set1_epi8(ske_code);//将16个有符号8位整数设置为ske_code
  repeat_sx_256 = _mm256_set1_epi8(ske_code);//将32个有符号8位整数设置为ske_code

  simd_scan_args_t* arg = (simd_scan_args_t*)malloc(sizeof(simd_scan_args_t));
  if (!arg) {
    printf("thread_simd_scan: failed to alloc for arg\n");
    exit(-1);
  }
  arg->target = target;
  arg->start = 0;
  arg->jobs = DATA_N;
  arg->is_unique = is_unique;

  simd_scan_work(arg);//此处使用arg存疑？
}
...
void test_scan(base_t target) {
  ...
  if (OPERATOR_TYPE == OPERATOR_BET)
    printf("finding %u < x < %u\n", TARGET_NUMBER, TARGET_NUMBER_2);
