...
#define SKE_N (1 << 8)  // typedef int8_t ske_t
#define FREQ_Z SKE_N
...
void init_data_by_random() {
  ...
  // data and samples
  for (int i = 0; i < size; i++) data[i] = dist(mt);
...
void append(){
  ...
  if (size_i == 0) {
    ...
    for (int pi = 0; pi < size_i; pi++) data[size+pi]  = file_data[pi];
    ...
    for (int pi = 0; pi < size_i; pi++) ske_col[size+pi]=map_it(file_data[pi]);
    size+=size_i;          
    }
